function arcrmodel_radial_2011_rates(Te_r_in, ne_r_in, r_in, xn0, T0, ion_fraction, rmin, rmax, numdivs, rates)
...
fileID = fopen('ar_atomic_rates.txt', 'w');


for nr = 1:(numdivs+1)
    for np = 1:Nuse
        %fprintf(fileID, '%f\t', Kloss(nr, np) + Kloss_ion(nr, np));
        fprintf(fileID, '%f\t', Kloss_ion(nr, np));
    end
...
for nr = 1:(numdivs+1)
    for np = 1:Nuse
        fprintf(fileID, '%f\t', Rgain_ion(nr, np));
    end
...
for nr = 1:(numdivs+1)
    for np = 1:Nuse
        for np2 = 1:Nuse
            fprintf(fileID, '%f\t', Kgain(nr, np2, np));
        end
