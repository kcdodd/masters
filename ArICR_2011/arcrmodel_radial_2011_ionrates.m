function arcrmodel_radial_2011_ionrates(Te_r_in, ne_r_in, r_in, xn0, T0, ion_fraction, rmin, rmax, numdivs, rates, inelastic_corrections, outfile)
...
for r_i = 1:(numdivs+1)
    ...
    for i = 1:size(inelastic_corrections.n0,2)
        ...
        if (inelastic_corrections.n0(i) > xn0*1E6)
            n0_interp_i = i-1;
            n0_interp_factor = (xn0*1E6 - inelastic_corrections.n0(i-1))/(inelastic_corrections.n0(i)-inelastic_corrections.n0(i-1));
            break;
        end
    ...
    correction_ion = (1-n0_interp_factor)*inelastic_corrections.Ar_I_ion_corrections(:,:,n0_interp_i) + n0_interp_factor*inelastic_corrections.Ar_I_ion_corrections(:,:,n0_interp_i+1);


disp('Precalculations done. Begining output.');
...
for nr = 1:(numdivs+1)
    ...
    if (ne_r(nr)*1E6 < inelastic_corrections.ne(1))
        ne_interp_i = 1;
        ne_interp_factor = 0;
    else
    
        for i = 1:size(inelastic_corrections.ne,2)

            if (inelastic_corrections.ne(i) > ne_r(nr)*1E6)
                ne_interp_i = i-1;
                ne_interp_factor = (ne_r(nr)*1E6 - inelastic_corrections.ne(i-1))/(inelastic_corrections.ne(i)-inelastic_corrections.ne(i-1));
                break;
            end
    ...
    if (Te_r(nr) < inelastic_corrections.Te(1))
        Te_interp_i = 1;
        Te_interp_factor = 0;
    else
    
        for i = 1:size(inelastic_corrections.Te,2)

            if (inelastic_corrections.Te(i) > Te_r(nr))
                Te_interp_i = i-1;
                Te_interp_factor = (Te_r(nr) - inelastic_corrections.Te(i-1))/(inelastic_corrections.Te(i)-inelastic_corrections.Te(i-1));
                break;
            end
    ...
    correction = (1-Te_interp_factor)*(1-ne_interp_factor)*correction_ion(Te_interp_i, ne_interp_i) + Te_interp_factor*(1-ne_interp_factor)*correction_ion(Te_interp_i+1, ne_interp_i) + (1-Te_interp_factor)*ne_interp_factor*correction_ion(Te_interp_i, ne_interp_i+1) + Te_interp_factor*ne_interp_factor*correction_ion(Te_interp_i+1, ne_interp_i+1);

    fprintf(fileID, '%E\r\n', correction*Kloss_ion(nr, 1));
end
