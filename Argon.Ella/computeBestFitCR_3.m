function [n0_best, sums_calc_best, actual_emission, lines, n_levels_best, n_levels_actual, R2] = computeBestFitCR_3(wavelengths, spectral_emission, n0_l, level_table)
...
%    n_levels_actual(4) = (actual_emission(1)^2/(lineData(1,2)*lineData(1,4)) + actual_emission(4)^2/(lineData(4,2)*lineData(4,4)) + actual_emission(12)^2/(lineData(16,2)*lineData(16,4)))/(actual_emission(1)+actual_emission(4)+actual_emission(12));
			...
			if (n_levels_actual(n) > 0)
                    		SSerr = SSerr + (level_table(n, n0_i) - n_levels_actual(n))^2;
			end
                end
    ...
    R2 = 1 - SSerr_best/SStot;
                ...
                for i = 1:25
                    calced_lines(i) = lineData(i,2)*lineData(i,4)*level_table(lineData(i,3), n0_best_i);
                end
