...
%   Detailed explanation goes here
    ...
    gain_density_integral = calc_integral(gain_density, dv);
    
    
    for e = 1:num_energies
        
        
        e_min = min(num_energies, ceil((ionization_energy + rate_energies(e) - rate_energies(1))/dE));
        
        % factor of 2 is because of contribution equally from freed electron and incident electron
        G(e) = 2*(gain_density_integral(num_energies) - gain_density_integral(e_min));
    end
    ...
    % and now for good measure make sure gain exactly balances total loss
    % rate = 2*L_0
